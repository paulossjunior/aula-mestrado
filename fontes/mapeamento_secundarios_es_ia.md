...
As oito fases usadas aqui estão definidas com critérios de inclusão, exclusão e teste de decisão em `codebook_fases_processo.md`.

## Protocolo

A varredura combinou 74 consultas ao arXiv organizadas em quatro blocos: consultas dedicadas a estudos secundários (survey, systematic literature review, systematic mapping, taxonomy, tertiary study), consultas por fase do ciclo de vida (requisitos, arquitetura, implementação, teste, reparo, manutenção), consultas por mecanismo agêntico (agentes autônomos, multi-agente, auto-evolução) e consultas sobre o lado humano e empírico do processo. Cada consulta retornou até 40 registros ordenados por relevância, consolidados por identificador arXiv. Isso produziu 1.172 registros únicos.

O filtro de segunda ordem operou sobre título e resumo, procurando marcadores explícitos de estudo secundário no título e declarações de método de revisão no resumo. Dos 300 candidatos, 188 tinham escopo de engenharia ou desenvolvimento de software; a triagem por modelo de linguagem descartou mais 31 cujo objeto central era outro domínio, o que deixa **157 estudos secundários** no corpus final. Cada um recebeu um rótulo de fase do processo, um rótulo de tipo de estudo e uma classificação do grau em que agentes autônomos são o objeto da revisão.

As métricas de citação vieram do OpenAlex, casadas por similaridade de título de pelo menos 0,85 para evitar o problema conhecido de resolução por DOI arXiv. Apenas 45 dos 157 estudos têm registro casado, e apenas 18 têm FWCI. Isso não é falha da busca: preprints recentes de engenharia de software raramente têm lista de referências indexada, e o desdobramento entre versão preprint e versão publicada subconta os trabalhos fundadores. As citações abaixo servem para separar clássicos consolidados de trabalhos ainda sem rastro bibliométrico, não para ranquear qualidade.

## Onde o campo já se revisou

![Distribuição dos estudos secundários por fase do processo e ano]({{artifact:b07c7f25-ec4c-4698-bc0e-b59280161763}})

| Fase do processo | 2023 | 2024 | 2025 | 2026 | Total |
|---|---|---|---|---|---|
| F1 Requisitos & especificação | 4 | 3 | 5 | 1 | 13 |
| F2 Projeto & arquitetura | 1 | 1 | 3 | 2 | 7 |
| F3 Implementação & geração de código | 3 | 12 | 19 | 9 | 43 |
| F4 Verificação & qualidade | 3 | 8 | 18 | 11 | 40 |
| F5 Depuração & reparo | 2 | 1 | 3 | 2 | 8 |
| F6 Manutenção, evolução & operação | 1 | 2 | 2 | 4 | 9 |
| F7 Processo transversal & ciclo completo | 4 | 8 | 15 | 6 | 33 |
| F8 Formação e educação em engenharia de software | – | 1 | 2 | 1 | 4 |

A distribuição é fortemente desequilibrada. Implementação e verificação juntas concentram 83 dos 157 estudos, mais da metade do corpus. Requisitos tem 13, projeto e arquitetura tem 7, manutenção e operação tem 9 e formação tem 4. Isso reproduz, na literatura de segunda ordem, o mesmo viés que os estudos primários já apresentavam: a atividade que o modelo de linguagem executa bem e cujo resultado é fácil de medir com um benchmark é também a atividade que mais atrai revisões.

A fase de processo transversal, com 33 estudos, é a que mais cresceu em termos relativos e a que concentra o olhar agêntico. Nela estão as revisões que tratam o agente como uma unidade organizacional dentro de um fluxo de trabalho, e não como um gerador de trechos de código: sistemas multi-agente para engenharia de software, avaliação de agentes, arquiteturas de memória e de contexto, e frameworks de raciocínio.

| Fase do processo | Agentes como objeto central | Agentes como seção | Sem foco em agência |
|---|---|---|---|
| F1 Requisitos & especificação | – | 8 | 5 |
| F2 Projeto & arquitetura | 1 | 3 | 3 |
| F3 Implementação & geração de código | 8 | 21 | 14 |
| F4 Verificação & qualidade | 8 | 19 | 13 |
| F5 Depuração & reparo | 2 | 5 | 1 |
| F6 Manutenção, evolução & operação | 3 | 5 | 1 |
| F7 Processo transversal & ciclo completo | 21 | 8 | 4 |
| F8 Formação e educação em engenharia de software | – | 3 | 1 |

Vinte e um dos 33 estudos de processo transversal têm agentes autônomos como objeto central, contra 8 em implementação e 8 em verificação. A leitura é direta: quando a revisão sobe da tarefa para o processo, ela necessariamente encontra agência, porque é no nível do processo que a autonomia se manifesta como escolha de próxima ação e não como qualidade de um trecho gerado.

## Que tipo de síntese está sendo produzida

| Tipo de estudo secundário | 2023 | 2024 | 2025 | 2026 | Total |
|---|---|---|---|---|---|
| T1 Revisão sistemática (SLR) | 3 | 10 | 18 | 12 | 43 |
| T2 Mapeamento sistemático | 4 | 4 | 7 | 2 | 17 |
| T3 Survey narrativo | 9 | 19 | 36 | 14 | 78 |
| T4 Taxonomia/framework | 1 | – | 1 | 1 | 3 |
| T5 Estudo terciário/meta-análise | – | – | – | 1 | 1 |
| T6 Survey com praticantes + revisão | 1 | 3 | 5 | 6 | 15 |

Metade do corpus são surveys narrativos. Revisões com protocolo explícito, somando revisões sistemáticas e estudos de mapeamento, chegam a 60, e a proporção melhorou de forma visível: em 2023 eram 7 de 18; em 2026 são 14 de 36. O aparecimento de 15 estudos que combinam revisão da literatura com dados primários de questionário aplicado a praticantes é o sinal metodológico mais interessante do período, porque é a única linha que confronta a literatura com o que equipes de fato fazem.

O corpus contém exatamente um estudo terciário e três trabalhos cuja contribuição central é uma taxonomia. Para um campo com 157 revisões em quatro anos, isso é pouco. A ausência de revisões de revisões significa que não há hoje uma síntese consolidada dizendo o que as revisões concordam entre si, e a repetição de escopo entre surveys de geração de código é evidência de que o trabalho está sendo refeito.
...
| [2608.01001](https://arxiv.org/abs/2608.01001) | 2026 | F6 | T2 | Manutenção e evolução de sistemas agenticos, dívida técnica em agentes autônomos | From AI Technical Debt to Agentic Technical Debt: A Systematic Mapping of Root Causes and Manifestations in Agentic AI Systems |
| [2605.15245](https://arxiv.org/abs/2605.15245) | 2026 | F7 | T1 | Agentes autônomos em todo ciclo de vida desenvolvimento software | Assistance to Autonomy: A Systematic Literature Review of Agentic AI across the Software Development Life Cycle |
...
| [2510.09721](https://arxiv.org/abs/2510.09721) | 2025 | F7 | T3 | Benchmarks e soluções em engenharia de software com sistemas agenticos LLM | A Comprehensive Survey on Benchmarks and Solutions in Software Engineering of LLM-Empowered Agentic System |
| [2508.00083](https://arxiv.org/abs/2508.00083) | 2025 | F7 | T3 | Agentes LLM autônomos para geração código e ciclo vida software | A Survey on Code Generation with LLM-based Agents |
| [2512.22256](https://arxiv.org/abs/2512.22256) | 2025 | F5 | T3 | Agentes LLM para resolução automática de issues em repositórios software | Agentic Software Issue Resolution with Large Language Models: A Survey |
...
| [2508.17692](https://arxiv.org/abs/2508.17692) | 2025 | F7 | T3 | Frameworks de raciocínio agentico baseados em LLM para engenharia de software | LLM-based Agentic Reasoning Frameworks: A Survey from Methods to Scenarios |
...
O mapeamento expõe cinco vazios que nenhuma das 157 revisões cobre de forma satisfatória.

Não existe estudo terciário do campo. Com 43 revisões sistemáticas e 17 estudos de mapeamento já publicados, há material suficiente para uma revisão de revisões que consolide as taxonomias concorrentes e identifique onde elas discordam. Esse é o trabalho de maior retorno imediato disponível.
...
Projeto e arquitetura estão sub-revisados em relação ao seu peso no processo. 7 revisões, das quais 5 de 2025 em diante, e nenhuma que trate decisão arquitetural feita por agente como fenômeno de processo em vez de tarefa de geração de diagrama.

A manutenção e a operação, com 9 revisões, são a fase em que o custo real do software se concentra e a que menos síntese recebeu. Migração de sistemas legados, dívida técnica introduzida por código gerado e operação assistida por agentes aparecem em revisões isoladas sem articulação entre si.

Falta articulação entre a literatura de processo agêntico e a literatura empírica sobre equipes. Os 15 estudos que coletam dados de praticantes raramente dialogam com os 33 estudos de processo transversal, o que deixa as propostas de fluxo agêntico sem contraparte de evidência sobre adoção.
